...
from app.services.rag.ocr import OcrPage, OcrResult
...
def test_low_confidence_image_ocr_creates_document_review(client: TestClient, monkeypatch) -> None:  # noqa: ANN001
    monkeypatch.setattr(
        "app.services.rag.parsers.ocr_document",
        lambda *_args, **_kwargs: OcrResult(
            pages=[OcrPage("Invoice image Total: EUR 12.00", 1)],
            confidence=0.58,
        ),
    )
