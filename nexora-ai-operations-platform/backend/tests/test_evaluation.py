...
def test_held_out_dataset_is_versioned_and_separate_from_regression_cases(client: TestClient) -> None:
    service = client.app.state.evaluation_service
    regression = service.load_cases()
    held_out = service.load_cases(service.held_out_cases_path)

    assert len(regression) == 40
    assert len(held_out) == 30
    assert {case.id for case in regression}.isdisjoint(case.id for case in held_out)

    response = client.post(
        "/api/v1/evals/run",
        json={"name": "held-out smoke", "dataset": "held_out", "configurations": ["improved"], "max_cases": 1},
    )
    assert response.status_code == 201, response.text
    dataset = response.json()["config"]["dataset"]
