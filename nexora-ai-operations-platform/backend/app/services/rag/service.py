...
def _expand_query(query: str) -> str:
    """Apply token-level domain vocabulary without matching benchmark sentences."""

    terms = _meaningful_terms(query)
    additions: list[str] = []
    if "card" in terms and terms & {"lost", "missing", "stolen", "theft"}:
        additions.append("freeze card emergency support fraud screening customer safety")
    if "card" in terms and terms & {"help", "route", "support", "contact", "emergency"}:
        additions.append("in-app emergency flow 24 hours 7 days")
    if "account" in terms and terms & {"access", "login", "locked", "signin", "recovery"}:
