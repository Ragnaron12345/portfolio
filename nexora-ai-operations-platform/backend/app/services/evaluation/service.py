...
class EvaluationService:
    def __init__(
        ...
        cases_path: Path,
        *,
        held_out_cases_path: Path | None = None,
        knowledge_lock: Any | None = None,
    ) -> None:
        self.processor = processor
        self.cases_path = cases_path
        self.held_out_cases_path = held_out_cases_path or cases_path.with_name("held_out.json")
    ...
    def run(self, db: Session, payload: EvalRunCreate) -> EvaluationRun:
        dataset_path = self.held_out_cases_path if payload.dataset == "held_out" else self.cases_path
        cases = self.load_cases(dataset_path) if payload.cases is None else payload.cases
    ...
    def _start_run(
        ...
        with self._knowledge_lock:
            ...
            dataset = {
                "name": (
                    ("Fintech support" if payload.dataset == "regression" else "Fintech support held-out")
                    if payload.cases is None
                    else "Inline evaluation cases"
                ),
                "split": payload.dataset if payload.cases is None else "inline",
    ...
    def load_cases(self, cases_path: Path | None = None) -> list[EvalCase]:
        cases_path = cases_path or self.cases_path
        paths: list[Path]
        if cases_path.is_dir():
            paths = sorted(cases_path.glob("*.json"))
        elif cases_path.exists():
            paths = [cases_path]
        else:
            parent = cases_path.parent
            paths = sorted(parent.glob("*.json")) if parent.exists() else []
        if not paths:
            raise EvaluationDatasetError(f"no evaluation JSON found at {cases_path}; pass cases in the request")
        cases: list[EvalCase] = []
        try:
            for path in paths:
